...
import com.sekai.ambienceblock.util.SharedConstants;
...
public class TextFieldWidget extends Widget implements IGuiEventHandler, IRenderable
{
    ...
    public TextFieldWidget(FontRenderer font, int x, int y, int width, int height, String text)
    {
        this(font, x, y, width, height, (TextFieldWidget)null, text);
    }

    public TextFieldWidget(FontRenderer font, int x, int y, int width, int height, @Nullable TextFieldWidget other, String text)
    {
    ...
    public boolean keyPressed(int typedChar, int keyCode, int modifiers)
    {
        if (!this.canWrite()) {
            ...
            this.isFocused = Screen.hasShiftDown();
            if (Screen.isSelectAll(typedChar)) {
                this.setCursorPositionEnd();
                this.setSelectionPos(0);
                return true;
            } else if (Screen.isCopy(typedChar)) {
                //Minecraft.getInstance().keyboardListener.setClipboardString(this.getSelectedText());
                return true;
            } else if (Screen.isPaste(typedChar)) {
                if (this.isEnabled) {
                    //this.writeText(Minecraft.getInstance().keyboardListener.getClipboardString());
                }

                return true;
            } else if (Screen.isCut(typedChar)) {
                //Minecraft.getInstance().keyboardListener.setClipboardString(this.getSelectedText());
                if (this.isEnabled) {
                    this.writeText("");
                }
    ...
    public boolean mouseClicked(double x, double y, int mouseButton)
    {
        if (!this.getVisible()) {
            ...
            boolean flag = x >= (double)this.x && x < (double)(this.x + this.width) && y >= (double)this.y && y < (double)(this.y + this.height);
            if (this.canLoseFocus) {
                this.setFocused2(flag);
            }

            if (this.isFocused() && flag && mouseButton == 0) {
                int i = MathHelper.floor(x) - this.x;
    ...
    public void renderButton(int x, int y, float partialTicks)
    {
        if (this.getVisible()) {
            ...
            if (flag1) {
                if (flag2) {
                    drawRect(k1, i1 - 1, k1 + 1, i1 + 1 + 9, -3092272);
                } else {
    ...
    public boolean isMouseOver(double x, double y) {
        return this.visible && x >= (double)this.x && x < (double)(this.x + this.width) && y >= (double)this.y && y < (double)(this.y + this.height);
    }
